...
namespace Tesco.ClubcardProducts.MCA.Web.Business.BusinessLogics
{
    public class BoostsAtTescoBC : IBoostsAtTescoBC
    {
        ...
        public bool IsUnSpentBoostTokensAvailable(long customerID)
        {
            ...
            try
            {
                bool bExchangeEnabled = this.IsExchangeEnabled();
                if (!bExchangeEnabled)
                {
                    _logData.RecordStep("ExchangeEnabled : false");
                    result = false;
                }

                bool bCurrenltyBCVEPeriod = this.IsCurrenltyBCVEPeriod();

                if (!bCurrenltyBCVEPeriod)
                {
                    _logData.RecordStep("CurrenltyBCVEPeriod :false");
                    result = false;
                }
                RewardAndToken rewardAndTokenDetails = this.GetRewardAndTokens(customerID);
