...
namespace Tesco.ClubcardProducts.MCA.Web.Business.BusinessLogics
{
    ...
    {        
        ...
        public ManageCardsViewModel GetManageCardsModel(List<HouseholdCustomerDetails> households)
        {
          ...
          //   logData.CaptureData("Request Object", households);
            try
            {
                ...
                households.ForEach(c => c.Cards.ForEach(ca => ca.LastUsed = checkTransactionTIme(ca)));
        ...
        private ManageCardsViewModel.LastusedState checkTransactionTIme(Clubcard card)
        {
            LogData _logData = new LogData();
            ManageCardsViewModel.LastusedState lastUsed = ManageCardsViewModel.LastusedState.NewJoinee;
            try
            {
                ...
                if (card.TransactionDateTime.Equals(DateTime.MinValue))
                {
                    lastUsed = ManageCardsViewModel.LastusedState.NewJoinee;
                }
                else if (card.TransactionDateTime < DateTime.Now.AddMonths(-6))
                {
                    lastUsed = ManageCardsViewModel.LastusedState.OlderThanThreshold;
                }
                else if (card.TransactionType.HasValue && card.TransactionType == transactionType.TryParse<Int32>())
                {
                    lastUsed = ManageCardsViewModel.LastusedState.TransactionTypeMatch;
                }
                else if (card.TransactionDateTime.HasValue)
                {
                    lastUsed = ManageCardsViewModel.LastusedState.ActualDate;
                }
                _logger.Submit(_logData);
                return lastUsed;
            }
