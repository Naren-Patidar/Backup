...
{
    public class PersonalDetailsBC : IPersonalDetailsBC
    {
        ...
        public PersonalDetailsBC(IServiceAdapter customerServiceAdapter,
                                 ...
                                 IServiceAdapter myAccountCustomerServiceAdapter,
                                 ...
                                 IConfigurationProvider configProvider)
        {
            ...
            _myAccountCustomerServiceAdapter = myAccountCustomerServiceAdapter;
            _logger = logger;
        ...
        public CustomerAddressVerificationModel GetMyAccountData(long dotcomCustomerID, Int64 customerID)
        {
            LogData _logData = new LogData();
            CustomerAddressVerificationModel customerData = new CustomerAddressVerificationModel();
            try
            {
                //--get personal details from Dotcom           

                MCARequest request;
                MCAResponse response;
                request = new MCARequest();
                request.Parameters.Add(ParameterNames.DOTCOM_CUSTOMER_ID, dotcomCustomerID);
                response = _myAccountCustomerServiceAdapter.Get<DotcomCustomerDetails>(request);
                DotcomCustomerDetails DotcomCustomerDetails = (DotcomCustomerDetails) response.Data;
                //--Get personal details from NGC
                CustomerFamilyMasterData customerFamilyMasterData = this.GetCustomerMasterData(customerID, 100);

                if (DotcomCustomerDetails == null || DotcomCustomerDetails.LastName == null || DotcomCustomerDetails.PostCode == null || customerFamilyMasterData == null ||
                    customerFamilyMasterData.CustomerData == null || customerFamilyMasterData.CustomerData.Count < 1)
                {
                    customerData.IsCustomerDetailsMatched = true;
                    return customerData;
                }               

                //--Compare Dotcom and NGC personal details
                //customerData.IsCustomerDetailsMatched = false;

                if ((DotcomCustomerDetails.LastName.Trim().ToUpper() == customerFamilyMasterData.CustomerData[0].LastName.Trim().ToUpper()) &&
                (GeneralUtility.removeEmptySpaceFromPostcode(DotcomCustomerDetails.PostCode.ToUpper()) 
                == GeneralUtility.removeEmptySpaceFromPostcode(customerFamilyMasterData.CustomerData[0].MailingAddressPostCode).ToUpper()))
                {
                    customerData.IsCustomerDetailsMatched = true;
                }
                else
                {
                    //--Clubcard data
                    customerData.lastName = customerFamilyMasterData.CustomerData[0].LastName;
                    string strPostCode = customerFamilyMasterData.CustomerData[0].MailingAddressPostCode;
                    //--Dotcom Data
                    customerData.dotcomLastName = DotcomCustomerDetails.LastName.Trim();
                    string strDotcomPostCode = DotcomCustomerDetails.PostCode.Trim();
                    if (_configProvider.GetStringAppSetting(AppConfigEnum.IsMaskingRequiredforPostcode) == "1")
                    {
                        customerData.Postcode = GeneralUtility.getFormattedPostcode(strPostCode);//--Clubcard data
                        customerData.dotcomPostcode = GeneralUtility.getFormattedPostcode(strDotcomPostCode);//--Dotcom Data
                    }
                    else
                    {
                        customerData.Postcode = strPostCode;//--Clubcard data
                        customerData.dotcomPostcode = strDotcomPostCode;//--Dotcom Data
                    }
                    customerData.IsCustomerDetailsMatched = false;
                }

                _logData.RecordStep(string.Format("Online Customer Details Matched : {0}", customerData.IsCustomerDetailsMatched));                
                _logger.Submit(_logData);

            }
            catch (Exception ex)
            {
                throw GeneralUtility.GetCustomException("Exception in getting GetMyAccountData data view", ex, 
        ...
        public CustomerFamilyMasterData GetCustomerMasterData(long customerId, int maxRows)
        {
            ...
            try
            {
                ...
                request.Parameters.Add(ParameterNames.OPERATION_NAME, OperationNames.GET_CUSTOMER_FAMILY_BY_CUSTID);
        ...
        public PersonalDetailsViewModel GetCustomerDataView(long customerId)
        {
            ...
            try
            {
                ...
                CustomerPreference custPreference = this.GetCustomerPreferences(customerId);
                ...
                {
                    string YearOfBirth = customerDateOfBirth.Year.ToString();
                    if (GeneralUtility.GetYearsList().Any(item => item.Value == YearOfBirth))
                    {
        ...
        public bool IsProfaneText(PersonalDetailsViewModel viewModel, long customerID, Dictionary<string, string> resourceKeys)
        {
            ...
            try
            {
                ...
                {
                    if (!IsProfaneText(PrepareProfaneText(hashCustomerDetails, customerID), customerID))
                    {
                        logData.RecordStep("Profanity check true");
                        _logger.Submit(logData);
                        return true;
                    }
                }
                logData.RecordStep("Profanity check false");
                _logger.Submit(logData);
            }
            catch (Exception ex)
            {
                throw GeneralUtility.GetCustomException("Exception in profanity check", ex, new Dictionary<string, object>() 
                                                                        { 
                                                                            { LogConfigProvider.EXCLOGDATAKEY, logData }
                                                                        });
            }
        ...
        /// <summary>
        /// Method for Populating address details
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public AddressDetails PopulateAddress(string postCodeInput)
        {
            ...
            try
            {
                DbConfigurationItem dbConfigurationItemForPostCode = _configProvider.GetConfigurations(DbConfigurationTypeEnum.Format, DbConfigurationItemNames.MailingAddressPostCode);
                string regPostCode = dbConfigurationItemForPostCode.ConfigurationValue1.ToString();
                string regPostCode1 = dbConfigurationItemForPostCode.ConfigurationValue2.ToString();
                string postCode = GeneralUtility.FormatPostalCode(postCodeInput);
                logData.RecordStep(string.Format("Getting addresses for Postcode {0}", postCodeInput));

                if (!RegexUtility.IsRegexMatch(postCode, regPostCode, false, false)
                || !RegexUtility.IsRegexMatch(postCode, regPostCode1, false, false))
                {
                    bErrorFlag = false;
                }
                ...
                if (bErrorFlag)
                {
                    List<AddressByPostCode> addressByPostCodeList = new List<AddressByPostCode>();
                    MCARequest request = new MCARequest();
                    request.Parameters.Add(ParameterNames.POST_CODE, postCode);
                    request.Parameters.Add(ParameterNames.OPERATION_NAME, OperationNames.GET_ADDRESSES_FOR_POST_CODE);

                    response = _locatorServiceAdapter.Get<List<AddressByPostCode>>(request);
                    logData.RecordStep(string.Format("Got addresses for Postcode {0}", postCodeInput));
                    if (response.Status)
                    {
                        addressByPostCodeList = response.Data as List<AddressByPostCode>;
                        if (addressByPostCodeList.Count > 0)
                        {
                            if (addressByPostCodeList[0].BuildingNumber.ToString() != string.Empty)
                            {
                                addressDetails = GetAddressBuildingNo(addressByPostCodeList);
                            }
                            else if (addressByPostCodeList[0].BuildingName.ToString() != string.Empty)
                            {
                                addressDetails = GetAddressBuildingName(addressByPostCodeList);
                            }

                            if (addressByPostCodeList[0].Street.ToString().Trim() != string.Empty)
                            {
                                if (addressByPostCodeList[0].SubBuilding.ToString().Trim() != string.Empty)
                                {
                                    if (addressByPostCodeList[0].BuildingNumber.ToString().Trim() != string.Empty)
                                    {
                                        addressDetails.Street = string.Format("{0} {1}", addressByPostCodeList[0].BuildingNumber.ToString().Trim(), addressByPostCodeList[0].Street.ToString().Trim());
                                    }
                                    else
                                    {
                                        addressDetails.Street = addressByPostCodeList[0].Street.ToString().Trim();
                                    }
                                }
                                else
                                {
                                ...
                                addressDetails.IsStreetVisible = true;
                            }
                            else
                            {
                                if (addressByPostCodeList[0].Locality.ToString().Trim() != string.Empty)
                                {
                                    addressDetails.IsStreetVisible = false;
                                }
                                else
                                {
                                    addressDetails.Street = string.Empty;
                                }
                            }
                            addressDetails.City = (!string.IsNullOrEmpty(addressByPostCodeList[0].Town.ToString().Trim())) ? addressByPostCodeList[0].Town.ToString().Trim() : string.Empty;
                            addressDetails.IsCityVisible = (!string.IsNullOrEmpty(addressByPostCodeList[0].Town.ToString().Trim())) ? true : false;
                            addressDetails.Locality = (!string.IsNullOrEmpty(addressByPostCodeList[0].Locality.ToString().Trim())) ? addressByPostCodeList[0].Locality.ToString().Trim() : string.Empty;
                            addressDetails.IsLocalityVisible = (!string.IsNullOrEmpty(addressByPostCodeList[0].Locality.ToString().Trim())) ? true : false;
                            addressDetails.IsCountryVisible = (!string.IsNullOrEmpty(addressByPostCodeList[0].Country.ToString().Trim())) ? true : false;
                            addressDetails.Country = (!string.IsNullOrEmpty(addressByPostCodeList[0].Country.ToString().Trim())) ? addressByPostCodeList[0].Country.ToString().Trim() : string.Empty;
                            addressDetails.IsErrorMessage = false;
                        }
                        else
                        {
                            addressDetails.IsErrorMessage = true;
                            addressDetails.Street = string.Empty;
                            addressDetails.City = string.Empty;
                        }
                    }
                    else
                    {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in populate address", ex, new Dictionary<string, object>() 
                                                                        { 
                                                                            { LogConfigProvider.EXCLOGDATAKEY, logData }
                                                                        });
            }
        ...
        /// <param name="customerId"></param>
        /// <returns></returns>
        public CustomerPreference GetCustomerPreferences(long customerId)
        {
            ...
            try
            {
                ...
                logData.CaptureData("Preferences", customerPreferences);
                _logger.Submit(logData);
            }
            catch (Exception ex)
            {
                throw GeneralUtility.GetCustomException("Exception in Getting customer preferences", ex, new Dictionary<string, object>() 
                                                                        { 
                                                                            { LogConfigProvider.EXCLOGDATAKEY, logData }
                                                                        });
            }
        ...
        /// <summary>
        /// validate if account is duplicate
        /// </summary>
        /// <param name="userData"></param>
        /// <returns></returns>
        public bool IsAccountDuplicate(PersonalDetailsViewModel viewModel, long customerID, Dictionary<string, string> resourceKeys)
        {
            LogData logData = new LogData();
            ...
            try
            {
                if (_configProvider.GetBoolAppSetting(AppConfigEnum.AccountDuplicateCheckRequired))
                {
                    userData = GetHashTableFromViewModel(viewModel, customerID, resourceKeys);
                    request = new MCARequest();
                    ...
                    if (response.Status)
                    {
                        if (context != null)
                        {
                            if (context.IsMainAccountUnique && context.IsAlternateAccountUnique)
                                isDuplicate = true;
                            logData.RecordStep("True for account duplicate check");
                        }
                    }
                    logData.RecordStep("False for account duplicate check");
                }
                else
                {
                    isDuplicate = true;
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Account duplicate check method", ex, new Dictionary<string, object>() 
                                                                        { 
                                                                            { LogConfigProvider.EXCLOGDATAKEY, logData }
                                                                        });
            }
        ...
        private bool UpdateCustomerDetails(Hashtable userData, Dictionary<string, string> resourceKeys, long customerId)
        {
            ...
            try
            {
                request = new MCARequest();
                request.Parameters.Add(ParameterNames.USER_DATA, userData);
                request.Parameters.Add(ParameterNames.DOTCOM_CUSTOMER, _configProvider.GetStringAppSetting(AppConfigEnum.ServiceConsumer));
                request.Parameters.Add(ParameterNames.OPERATION_NAME, OperationNames.UPDATE_CUSTOMER_DETAILS);

                response = _customerServiceAdapter.Set<bool>(request);
        ...
        public bool UpdateCustomerDetails(CustomerFamilyMasterDataUpdate customerData)
        {
            bool isUpdatedCustomerDetails = false;
        ...
        public CustomerFamilyMasterData GetCustomerDetails(long customerId)
        {
            ...
            try
            {
                ...
                request.Parameters.Add(ParameterNames.OPERATION_NAME, OperationNames.GET_CUSTOMER_FAMILY_BY_CUSTID);
                response = _customerServiceAdapter.Get<CustomerFamilyMasterData>(request);
                customerFamilyData = (CustomerFamilyMasterData)response.Data;
                _logger.Submit(logData);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Update Customer Preferences Data in DB
        /// </summary>
        /// <param name="customerId"></param>
        /// <param name="objcustPref"></param>
        /// <param name="customerDetails"></param>
        /// <returns></returns>
        private bool UpdateCustomerPreferences(long customerId, CustomerPreference objcustPref, AccountDetails customerDetails)
        {
            bool isUpdatedCustomerPreferences = false;
            LogData logData = new LogData();
            try
            {
                request = new MCARequest();
                ...
                response = _preferenceServiceAdapter.Execute(request);

                if (response.Status)
                {
                    isUpdatedCustomerPreferences = (bool)response.Status;
                    logData.RecordStep(string.Format("Updated customer preferences with status {0}", isUpdatedCustomerPreferences));
                }
                else
                {
                    logData.RecordStep(string.Format("Updated customer preferences with status {0}", isUpdatedCustomerPreferences));
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Update customer preferences", ex, new Dictionary<string, object>() 
                                                                        { 
                                                                            { LogConfigProvider.EXCLOGDATAKEY, logData }
                                                                        });
            }
            return isUpdatedCustomerPreferences;
        }

        /// <summary>
        /// Send Email Notice to Customer based on prefrence option
        /// </summary>
        /// <param name="CustomerID"></param>
        /// <param name="objcustPref"></param>
        /// <param name="customerdetails"></param>
        ...
        /// <returns></returns>
        private bool SendEmailNoticeToCustomers(long customerId, CustomerPreference objcustPref, AccountDetails customerDetails, string PageName, Hashtable trackHT)
        {
            bool isEmailNoticeSent = false;
            LogData logData = new LogData();
            try
            {
                request = new MCARequest();
                ...
                request.Parameters.Add(ParameterNames.TRACKHT, trackHT);
                ...
                if (response.Status)
                {
                    isEmailNoticeSent = (bool)response.Data;
                    logData.RecordStep(string.Format("Sent email notice  to customers with status {0}", isEmailNoticeSent));
                }
                else
                {
                    logData.RecordStep(string.Format("Sent email notice  to customers with status {0}", isEmailNoticeSent));
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Send Email Notice to customer", ex, new Dictionary<string, object>() 
                                                                        { 
                                                                            { LogConfigProvider.EXCLOGDATAKEY, logData }
                                                                        });
            }
            return isEmailNoticeSent;
        }
        ...
        /// <param name="customerdetails"></param>
        /// <returns></returns>
        private bool SendEmailToCustomers(CustomerPreference objcustPref, AccountDetails customerDetails)
        {
            LogData logData = new LogData();
            ...
            try
            {
                ...
                if (response.Status)
                {
                    isEmailSent = (bool)response.Data;
                    logData.RecordStep(string.Format("Sent Email to customers with status {0}", isEmailSent));
                }
                else
                {
                    logData.RecordStep(string.Format("Sent Email to customers with status {0}", isEmailSent));
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Send email to customer", ex, new Dictionary<string, object>() 
                                                                        { 
                                                                            { LogConfigProvider.EXCLOGDATAKEY, logData }
                                                                        });
            }
        ...
        /// <returns></returns>
        private bool IsProfaneText(string text, long customerId)
        {
            LogData logData = new LogData();
            ...
            try
            {
                ...
                if (response.Status)
                {
                    ...
                    logData.RecordStep(string.Format("Profane service returns {0}", isProfaneText));
                }
                else
                {
                    logData.RecordStep(string.Format("Profane service returns {0}", isProfaneText));
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Is Profane text", ex, new Dictionary<string, object>() 
                                                                        { 
                                                                            { LogConfigProvider.EXCLOGDATAKEY, logData }
                                                                        });
            }
