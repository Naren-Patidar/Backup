...
namespace Tesco.ClubcardProducts.MCA.Web.Common.Validator
{
    public class Validators : ValidationAttribute
    {
        ...
        public Validators()
        {
            ...
            IsPersonalDetails = !(ctrl.ToLower().Equals("accountmanagement")) ? false : true;
            if (IsJoin)
            ...
            switch (moduleName)
            {
                ...
                case "Activation":
                    ...
                    if (value != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (vr == null)
                            {
                                if (displayName.ToString().Equals("FirstName"))
                                {
                                    displayName = "FirstNameForActivation";
                                }
                ...
                case "PersonalDetails":
                    ...
                    if (!ValidatorUtility.IsNotVisible(displayName))
                    {
                        ...
                        if (vr == null && !string.IsNullOrEmpty(Convert.ToString(value).Trim()))
                        {
        ...
        {
            ValidationResult vr = null;
            string value = actualValue != null ? actualValue.ToString() : String.Empty;
            if (DisplayName == "MailingAddressPostCode")
            {
                value = GeneralUtility.FormatPostalCode(value);
            }
            bool isValid = false;
        ...
        private ValidationResult LengthValidation(string DisplayName, object value)
        {
            ...
            string actualValue = value != null ? value.ToString() : String.Empty;
            if (DisplayName == "MailingAddressPostCode")
            {
                actualValue = GeneralUtility.FormatPostalCode(actualValue);
            }
            if (!string.IsNullOrEmpty(configValue1) && string.IsNullOrEmpty(configValue2))
            {
                if (!string.IsNullOrEmpty(actualValue) && actualValue.Trim().Length >= configValue1.TryParse<Int64>())
                    isValid = true;
                if (!isValid)
                {
            ...
            else if (!string.IsNullOrEmpty(actualValue) && string.IsNullOrEmpty(configValue1) && !string.IsNullOrEmpty(configValue2))
            {
                if (actualValue.Trim().Length <= configValue2.TryParse<Int64>())
                    isValid = true;
                if (!isValid)
                {
                    vr = new ValidationResult(HttpContext.GetLocalResourceObject(_viewpath, "Len_" + DisplayName,
            ...
            else if (!string.IsNullOrEmpty(actualValue) && !string.IsNullOrEmpty(configValue1) && !string.IsNullOrEmpty(configValue2))
            {
                ...
                if (actualValue.Trim().Length >= configValue1.TryParse<Int64>() && value.ToString().Trim().Length <= configValue2.TryParse<Int64>())
                    isValid = true;
                if (!isValid)
                {
                    vr = new ValidationResult(HttpContext.GetLocalResourceObject(_viewpath, "Len_" + DisplayName,
        ...
        private ValidationResult prefixValidation(string DisplayName, object value)
        {
            ...
            if (DisplayName.ToLower().Equals("mobilephonenumber"))
            {
                if (!string.IsNullOrEmpty(phoneNumber))//If the configured data has more than one value(comma seperated)
                {
                    ...
                    else if (phoneNumber.Trim().Length < configValue1.Trim().Length || phoneNumber.Trim().Substring(0, configValue1.Trim().Length) != configValue1)
                    {
