...
namespace Tesco.ClubcardProducts.MCA.Web.ServiceAdapter.Services
{
    ...
    public class ClubcardServiceAdapter : IServiceAdapter
    {
        ...
        public ILoggingService _logger = null;
        private IConfigurationProvider _Config;

        public ClubcardServiceAdapter(IClubcardService clubcardServiceClient, ILoggingService logger, IConfigurationProvider config)
        {
        ...
        public MCAResponse Get<T>(MCARequest req)
        {
            ...
            if (req.Parameters.Keys.Contains(ParameterNames.MODEL))
            {
                OrderReplacementModel model = (OrderReplacementModel)req.Parameters[ParameterNames.MODEL];
            ...
            try
            {
                ...
                switch (operation)
                {
                    case OperationNames.GET_OFFERS_FOR_CUSTOMER:
                        if (req.Parameters.Keys.Contains(ParameterNames.CUSTOMER_ID_Points) &&
                            req.Parameters.Keys.Contains(ParameterNames.CONDITIONAL_XML) &&
                            req.Parameters.Keys.Contains(ParameterNames.CULTURE) &&
                            req.Parameters.Keys.Contains(ParameterNames.MAX_ROWS))
                        {
                            res.Data = this.GetOffersForCustomer(
                                req.Parameters[ParameterNames.CUSTOMER_ID_Points].TryParse<Int64>(),
                                req.Parameters[ParameterNames.CONDITIONAL_XML].TryParse<string>(),
                                req.Parameters[ParameterNames.CULTURE].TryParse<string>(),
                                req.Parameters[ParameterNames.MAX_ROWS].TryParse<Int32>());
                            res.Status = true;
                        }
                        break;
                    ...
                    case OperationNames.GET_CHRISTMAS_SAVER_SUMMARY:
                        if (req.Parameters.Keys.Contains(ParameterNames.CUSTOMER_ID) &&
                            req.Parameters.Keys.Contains(ParameterNames.START_DATE) &&
                            req.Parameters.Keys.Contains(ParameterNames.END_DATE) &&
                            req.Parameters.Keys.Contains(ParameterNames.CULTURE))
                        {
                            res.Data = this.GetChristmasSaverSummary(
                                                        req.Parameters[ParameterNames.CUSTOMER_ID].TryParse<Int64>(),
                                                        req.Parameters[ParameterNames.START_DATE].TryParse<DateTime>(),
                                                        req.Parameters[ParameterNames.END_DATE].TryParse<DateTime>(),
                                                        req.Parameters[ParameterNames.CULTURE].TryParse<string>());
                            res.Status = true;
                        }
                        break;
                    ...
                    case OperationNames.GET_CUSTOMER_ACTIVATION_STATUS_DETAILS:
                        if (req.Parameters.Keys.Contains(ParameterNames.DOTCOM_CUSTOMER_ID)
                            && req.Parameters.Keys.Contains(ParameterNames.CULTURE))
                        {
                            res.Data = IGHSCheckCustomerActivatedStatus(
                                                req.Parameters[ParameterNames.DOTCOM_CUSTOMER_ID].ToString(),
                    ...
                    case OperationNames.GET_HOUSEHOLD_CUSTOMER_DATA:
                        if (req.Parameters.Keys.Contains(ParameterNames.CUSTOMER_ID) && req.Parameters.Keys.Contains(ParameterNames.CULTURE))
                        {
                    ...
                    case OperationNames.GET_POINTS_SUMMARY:
                        if (req.Parameters.Keys.Contains(ParameterNames.CUSTOMER_ID) &&
                            req.Parameters.Keys.Contains(ParameterNames.OFFER_ID) &&
                            req.Parameters.Keys.Contains(ParameterNames.CULTURE))
                        {
                            res.Data = this.GetPointsSummary(
                                                    req.Parameters[ParameterNames.CUSTOMER_ID].TryParse<Int64>(),
                                                    req.Parameters[ParameterNames.OFFER_ID].TryParse<Int32>(),
                                                    req.Parameters[ParameterNames.CULTURE].TryParse<string>());
                            res.Status = true;
                        }
                        break;
                    case OperationNames.GET_CUSTOMER_TRANSACTIONS:
                        if (req.Parameters.Keys.Contains(ParameterNames.CUSTOMER_ID) &&
                            req.Parameters.Keys.Contains(ParameterNames.OFFER_ID) &&
                            req.Parameters.Keys.Contains(ParameterNames.MERCHANTFLAG) &&
                            req.Parameters.Keys.Contains(ParameterNames.CULTURE))
                        {
                            res.Data = this.GetCustomerTransactions(
                                            req.Parameters[ParameterNames.CUSTOMER_ID].TryParse<Int64>(),
                                            req.Parameters[ParameterNames.OFFER_ID].TryParse<Int32>(),
                                (req.Parameters[ParameterNames.MERCHANTFLAG]).TryParse<Boolean>(),
                                req.Parameters[ParameterNames.CULTURE].TryParse<String>());
                            res.Status = true;
                        }
                        break;
                    case OperationNames.GET_ORDER_REPLACEMENT_STATUS:
                        {
                            if (req.Parameters.Keys.Contains(ParameterNames.CUSTOMER_ID) && req.Parameters.Keys.Contains(ParameterNames.CULTURE))
                            {
                                res.Data = this.GetOrderReplacementExistingStatus((req.Parameters[ParameterNames.CUSTOMER_ID]).TryParse<Int64>(), req.Parameters[ParameterNames.CULTURE].ToString());
                                res.Status = true;
                            }
                        }
                        break;
                    case OperationNames.GET_PROCESS_ORDER_REPLACEMENT_REQUEST:
                        {
                            if (req.Parameters.Keys.Contains(ParameterNames.MODEL))
                            {
                                res.Data = this.ProcessOrderReplacementRequest((OrderReplacementModel)req.Parameters[ParameterNames.MODEL]);
                                res.Status = true;
                            }
                        }
                        break;
            ...
            {
                throw GeneralUtility.GetCustomException("Failed in Clubcard Adapter|GET", ex,
        ...
        public DataSet GetHouseHoldCustomersDataSet(long customerID, string culture)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception while getting Customer's household Data set", ex,
              new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private DataSet GetClubcardsCustomerDataSet(long customerID, string culture)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw GeneralUtility.GetCustomException("Exception while getting Clubcard Customer Dataset", ex,
              new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private DataSet GetMyAccountDetailsDataset(long customerID, string culture)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw GeneralUtility.GetCustomException("Exception while getting My Account Details Dataset", ex,
              new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private List<CustomerMLS_PDF_DownloadDetails> GetMyAccountDetailsDatasetToList(DataSet dsCustomer)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in GetMyAccountDetailsDatasetToList", ex,
             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private List<HouseholdCustomerDetails> GetHouseHoldCustomers(DataSet dsHHCustomers, long customerID)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Failed in Clubcard Service Adapter while getting Household customers", ex, new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
            }
        ...
        private DataSet GetTransactionDetailsByCustomerAndOfferIDDataset(long customerId, int offerId, bool showMerchantFlag, string culture)
        {
            ...
            try
            {
                ...
                logData.CaptureData("Response Object", dsTransactions);
                _logger.Submit(logData);
            }
            catch (Exception ex)
            {

                throw GeneralUtility.GetCustomException("Exception in Clubcard Service Adapter while getting transactions data", ex,
                new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private CustomerTransactions GetTransactionDetailsByCustomerAndOfferIDDatasetToList(DataSet dsTransactionsAndOffer, long customerId)
        {
            ...
            try
            {
                ...
                logData.CaptureData("Response Object", customerTransactionDetails);
                _logger.Submit(logData);
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Getting transactionbycustomerandofferid", ex,
             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private List<HouseholdCustomerDetails> GetHouseHoldCustomers(long customerID, string culture)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Getting Household customers details", ex,
             new Dictionary<string, object>() 
                            { 
        ...
        private CustomerTransactions GetTransactionDetailsByCustomerAndOfferID(long customerId, int offerId, bool showMerchantFlag, string culture)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Getting Transaction details based on Customer and OfferID", ex,
             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, _logData }
                            });
        ...
        private List<Clubcard> GetMyAccountDetails(long customerID, string culture)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in getting My Account Details", ex,
             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, _logData }
                            });
        ...
        private List<Clubcard> GetMyAccountDetailsEntities(DataSet dsCustomer, long customerID)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in getting My Account Details Entities", ex,
             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, _logData }
                            });
        ...
        public List<ChristmasSaverSummary> GetChristmasSaverSummary(long customerID, DateTime startDate, DateTime endDate, string culture)
        {
            ...
            try
            {
                ...
                XmasSaverSummary["StartDate"] = startDate.ToString(appsettings[AppConfigEnum.DisplayDateFormat.ToString()].ConfigurationValue1) ;
                XmasSaverSummary["EndDate"] = endDate.ToString(appsettings[AppConfigEnum.DisplayDateFormat.ToString()].ConfigurationValue1); 
                ...
                logData.CaptureData("Response Object", christmasSaverDetailsList.christmasSaverSummaryList);
                _logger.Submit(logData);
                return christmasSaverDetailsList.christmasSaverSummaryList;
            }
            catch (Exception ex)
            {
                throw GeneralUtility.GetCustomException("Exception in Getting christmas saver summary", ex,
            new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private List<Offer> GetOffersForCustomer(long customerId, string conditionalXml, string culture, int maxRowCount)
        {
            ...
            try
            {
                ...
                logData.CaptureData("Response ", offers.OfferDetailList);
                _logger.Submit(logData);
                //return lstOffers;
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in Getting OffersForCustomer", ex,
            new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, customerId }
                            });
            }
        }
        ...
        private PointsSummary GetPointsSummary(long customerId, int previousOfferId, string culture)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception while getting Point Summary", ex,
                             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private CustomerTransactions GetCustomerTransactions(long customerId, int offerId, bool showMerchantFlag, string culture)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception while getting Customer Transactions", ex,
                             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private AccountDetails GetCustomerAccountDetails(long customerId, string culture)
        {
            ...
            {
                ex.Data.Add(LogConfigProvider.EXCLOGDATAKEY, logData);
                throw GeneralUtility.GetCustomException("Exception in customeraccountdetails", ex,
             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private bool IsFuelSaverAccountExists(string clubCardId)
        {
            ...
            try
            {
                ...
                {
                    _logData.CaptureData("Response", true);
                    _logger.Submit(_logData);
                    return true;
                }
                else
                {
                    _logData.CaptureData("Response", false);
                    _logger.Submit(_logData);
                    return false;
                }
            }
            ...
            {
                throw GeneralUtility.GetCustomException("Failed in Clubcard Service Adapter while getting status for IsFuelSaverAccountExists", ex, new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, _logData }
                            });
            }
        ...
        private AccountDetails GetMyCustomerAccountDetails(long customerId, string culture)
        {
            ...
            {
                throw GeneralUtility.GetCustomException("Exception while getting my customer account details", ex,
               new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private DataSet GetOrderReplacementExistingStatusDataSet(long customerId)
        {
            ...
            try
            {
                ...
                logData.RecordStep(string.Format("conditionXml: {0}", conditionXml));
            ...
            catch (Exception ex)
            {
                ...
                throw GeneralUtility.GetCustomException("Exception while getting OrderReplacement Existing Status Data", ex,
             new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private bool ProcessOrderReplacementRequest(OrderReplacementModel model)
        {
            ...
            try
            {
                if (model.ClubcardNumber > 0)
                {
                    ...
                    logData.CaptureData("Response Object", isServiceSuccessful);
                    _logger.Submit(logData);
                    return isServiceSuccessful;
                }
                else
                {
                    throw new Exception("Passing invalid clubcard number - Clubcard Number: " + model.ClubcardNumber);
                }
            }
            catch (Exception ex)
            {
                throw GeneralUtility.GetCustomException("Exception while processing Order Replacement Request", ex,
          new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private bool IsXmasClubMember(long customerId, string culture)
        {
            ...
            try
            {
                ...
                if (_clubcardServiceClient.IsXmasClubMember(out errorXml, out resultXml, customerId, culture))
                {
                    ...
                    {
                        logData.CaptureData("Response object", dsXmasClubMember.Tables[0].Rows[0][0].ToString());
                        _logger.Submit(logData);
            ...
            {
                throw GeneralUtility.GetCustomException("Exception in XmasClubMember", ex,
            new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
        ...
        private Activation.CustomerActivationStatusdetails IGHSCheckCustomerActivatedStatus(string dotcomCustomerID, string culture)
        {
            ...
            try
            {
                ...
                logData.CaptureData("Response Object", customerActivationStatus);
                _logger.Submit(logData);
            }
            catch (Exception ex)
            {
                throw GeneralUtility.GetCustomException("Exception in checking IGHS Customer ActivatedStatus", ex,
            new Dictionary<string, object>() 
                            { 
                                { LogConfigProvider.EXCLOGDATAKEY, logData }
                            });
