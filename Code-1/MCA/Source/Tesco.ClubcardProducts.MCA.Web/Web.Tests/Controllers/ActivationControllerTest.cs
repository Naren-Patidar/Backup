...
namespace Web.Tests
{
    ...
    public class ActivationControllerTest
    {
        ...
        private IActivationBC _IActivationBC;
        [SetUp]
        public void SetUp()
        {
            ...
            _IActivationBC = MockRepository.GenerateMock<IActivationBC>();
            // _logger = MockRepository.GenerateMock<ILoggingService>();
            activationController = new ActivationController(_IActivationBC, _IAccountBC, _AuditLoggerService, _configProvider);
        }
        [TestCase]
        public void ActivationDetails_Audit_Successful()
        {
            string strToCompare = StubAccountActivationData();
        ...
        private string StubAccountActivationData()
        {
            ...
            try
            {
                strToCompare = String.Format("ClubcardDetailsDoesntMatch|{0}",
                               new
                               {
                                   ClubcardNumber = 96888834532145,
                                   FirstName = "Alen",
                                   Surname = "Mark",
                                   Postcode = "ASD23 RT"
                               }.JsonText());
