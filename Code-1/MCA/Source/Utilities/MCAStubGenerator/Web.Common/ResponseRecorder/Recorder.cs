...
namespace Tesco.ClubcardProducts.MCA.Web.Common.ResponseRecorder
{
    public class Recorder
    {
        ...
        {
            string dirPath = Path.Combine(this._rootDir, serviceName, "DataSource", methodName);
            ...
            {
                string filePath = Path.Combine(dirPath, this._customerID + "-activated.txt");
            ...
            {
                string filePath = Path.Combine(dirPath, this._customerID + "-customerID.txt");
                if (File.Exists(filePath))
                {
                    File.Delete(filePath);
                }
            ...
            {
                string filePath = Path.Combine(dirPath, this._customerID + "-data" + rType.ToString());
                if (File.Exists(filePath))
                {
                    File.Delete(filePath);
                }
            ...
            {
                string errorfilePath = Path.Combine(dirPath, this._customerID + "-error" + rType.ToString());
            ...
            if (rLog.RowCount > 0)
            {
                string rowCountfilePath = Path.Combine(dirPath, this._customerID + "-rowCount.txt");
