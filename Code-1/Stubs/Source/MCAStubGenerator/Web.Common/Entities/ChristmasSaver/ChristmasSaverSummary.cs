    ...
    public class ChristmasSaverSummaryList : BaseEntity<ChristmasSaverSummaryList>
    {
        ...
        public override void ConvertFromXml(string xml)
        {
            ...
            _christmasSaverSummary = 
                ...
                {
                    TransactionDateTime = t.Element(ChristmasSaverSummaryEnum.TransactionDateTime.ToString()).GetValue<string>().TryParseDate(out dtTemp) ? dtTemp : DateTime.Now,
                    AmountSpent = Convert.ToDecimal(t.Element(ChristmasSaverSummaryEnum.AmountSpent.ToString()).GetValue<string>(), CultureInfo.InvariantCulture).ToString()
                }).ToList();
        }
    }
}
