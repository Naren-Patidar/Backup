...
{
    public class SKAccountDuplicacyCheck : AccountDuplicacyCheck
    {
        ...
        public override bool IsAccountDuplicate(long ClubcardNumber, ClubcardOnline.Web.Entities.CustomerActivationServices.ClubcardCustomer customer, System.Data.DataSet dsConfig)
        {
            ...
            try
            {
                ...
                fileName = Path.Combine(ServiceName, string.Format(@"DataSource\GetCustomerDetails\sk-SK\{0}.xml", ClubcardNumber));

                // fileName = string.Format(@"D:\MCA_RefactoredCode\UK\Stub_Jan2016\ClubcardService\DataSource\GetCustomerDetails\sk-SK\{0}.xml", ClubcardNumber);
            ...
            {
                throw Extensions.GetCustomException("Failed in SKAccountDuplicacy check while matching CustomerData.", ex, new Dictionary<string, object>() 
                            { 
