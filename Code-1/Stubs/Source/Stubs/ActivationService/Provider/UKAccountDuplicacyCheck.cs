...
{
    public class UKAccountDuplicacyCheck : AccountDuplicacyCheck
    {
        ...
        public override bool IsAccountDuplicate(long ClubcardNumber, ClubcardOnline.Web.Entities.CustomerActivationServices.ClubcardCustomer customer, System.Data.DataSet dsConfig)
        {
            ...
            try
            {
                ...
                fileName = Path.Combine(ServiceName, string.Format(@"DataSource\GetCustomerDetails\en-GB\{0}.xml", ClubcardNumber));
                _logData.CaptureData("FileName", fileName);
                //   fileName = string.Format(@"D:\MCA_RefactoredCode\UK\Stub_Jan2016\ClubcardService\DataSource\GetCustomerDetails\en-GB\{0}.xml", ClubcardNumber);
                    ...
                    if (responseNode.Name != null)
                    {
                        switch (value["ConfigurationName"].ToString())
                        {
                            case "MailingAddressPostCode":
                                if (!(responseNode.InnerText.Trim().ToLower() == customer.Address.PostCode.Trim().ToLower()))
                                {
                                    return false;
                                }
                                break;
                            case "Name1":
                                if (!(responseNode.InnerText.Trim().ToLower() == customer.FirstName.Trim().ToLower()))
                                {
                                    return false;
                                }
                                break;
                            case "Name3":
                                if (!(responseNode.InnerText.Trim().ToLower() == customer.Surname.Trim().ToLower()))
                                {
                                    return false;
                                }
                                break;
            ...
            {
                throw Extensions.GetCustomException("Failed in UKAccountDuplicacy check while matching CustomerData.", ex, new Dictionary<string, object>() 
                            { 
