...
set "File=STG(Dev)_P1Set4_MF_UK_Reskin_%YYYY%%MMM%%DD%_%HH%%Min%%Sec%.trx"
set "TRX_Report=%TRX_Path%%File%
set "Report_EXE=%Root_Path%Reporting\ReportGenerator.exe"
set "HTML_Report_Path=%Root_Path%Reports\UK_STG(Dev)\P1Set4\"
set "HTML_File=UK_MF_STG(Dev)_P1Set4_Reskin_%YYYY%%MMM%%DD%_%HH%%Min%%Sec%.html"

echo Starting the Test Run
@echo:
%mstestPath% /testcontainer:%Framework% /category:P1Set4 /resultsfile:%TRX_Report% /testsettings:%localfile%
