...
namespace Tesco.Framework.UITesting.Services
{
    public class ClubcardCouponAdaptor
    {
        ...
        public List<string> GetUsedCouponUsedIssuedDate(string clubcardNumber, string culture)
        {
            ...
            using (ClubcardCouponServiceClient client = new ClubcardCouponServiceClient())
            {
                ...
                client.GetRedeemedCoupons(out errorXml, out coupons, householdId, culture);
                XDocument xDoc = XDocument.Parse(coupons);
                IssuedDate = (from t in xDoc.Descendants("UsedCouponDetail")
                             select t.Element("IssuanceStartDate").GetValue<string>().Substring(0, 10)).ToList(); 
            }
            return IssuedDate;
        }


        public List<string> GetUsedCouponUsedOffer(string clubcardNumber, string culture)
        {
            ...
            using (ClubcardCouponServiceClient client = new ClubcardCouponServiceClient())
            {
                ...
                client.GetRedeemedCoupons(out errorXml, out coupons, householdId, culture);
                XDocument xDoc = XDocument.Parse(coupons);
                Offer = (from t in xDoc.Descendants("UsedCouponDetail")
                             select t.Element("CouponDescription").GetValue<string>()).ToList();
            }
            return Offer;
        }

        public List<string> GetUsedCouponUsedRedemption(string clubcardNumber, string culture)
        {
            List<string> Redeemption = new List<string>();
            CustomerServiceAdaptor customerAdaptor = new CustomerServiceAdaptor();
            Int64 householdId = customerAdaptor.GetHouseholdID(clubcardNumber, culture);
            using (ClubcardCouponServiceClient client = new ClubcardCouponServiceClient())
            {
                ...
                client.GetRedeemedCoupons(out errorXml, out coupons, householdId, culture);
                XDocument xDoc = XDocument.Parse(coupons);
                Redeemption = (from t in xDoc.Descendants("UsedCouponDetail")
                         select t.Element("RedemptionCount").GetValue<string>()).ToList();
            }
            return Redeemption;
        }

        public List<string> GetUsedCouponUsedUsedVoided(string clubcardNumber, string culture)
        {
            List<string> Redeemption = new List<string>();
            CustomerServiceAdaptor customerAdaptor = new CustomerServiceAdaptor();
            Int64 householdId = customerAdaptor.GetHouseholdID(clubcardNumber, culture);
            using (ClubcardCouponServiceClient client = new ClubcardCouponServiceClient())
            {
                ...
                XDocument xDoc = XDocument.Parse(coupons);
                Redeemption = (from t in xDoc.Descendants("UsedCouponDetail")
                               select t.Element("CouponStatus").GetValue<string>()).ToList();
                if (Redeemption.Contains("Redeem"))
                {
                }
            }
            return Redeemption;
