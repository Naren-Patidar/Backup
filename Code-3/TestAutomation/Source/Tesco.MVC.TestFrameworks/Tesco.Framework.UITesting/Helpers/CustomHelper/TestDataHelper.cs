...
namespace Tesco.Framework.UITesting.Helpers.CustomHelper
{
    ...
    {
        public static string GetTestData(string xmlPath, string module, string key, string domain)
        {
            ...
            try
            {
                ...
                xList = (from t in xData.Descendants(module)
                         where t.Attribute("ID").Value.ToUpper().Equals(domain.ToUpper())
                         select t.Element(XName.Get(key))).ToList();
                XElement node = xList.FirstOrDefault();
