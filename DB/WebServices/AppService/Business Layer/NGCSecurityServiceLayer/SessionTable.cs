...
{
    public class SessionTable : Hashtable, IDisposable
    {
        ...
        /// </summary>
        public SessionTable(Trace trace)
        {

            TraceState trState = trace.StartProc("SessionTable.SessionTable Instance");
            try
            {
            ...
            out Session newSession)
        {
            ...
            TraceState trState = trace.StartProc("SessionTable.Add UserName=" + userName);
            newSession = null;
            string sessionId;
            try
            {
                try
                {
                    ...
                    newSession = new Session(trace, sessionId, userName, password, culture, null);
            ...
            out string capabilityXml)
        {
            ...
            try
            {
                ...
                {
                    sessionId = Guid.NewGuid().ToString();
                    newSession = new Session(trace, sessionId, userName, password, culture, AppName);
                    capabilityXml = newSession.CapabilityXml;
                    Int32 WaitInterval = Convert.ToInt32(ConfigurationSettings.AppSettings["WaitInterval"]);
                    this.RwLock.AcquireWriterLock(WaitInterval);
                    try
        ...
        private void RemoveInactiveSessions(object sender, ElapsedEventArgs e)
        {
            ...
            TraceState trState = trace.StartProc("SessionTable.RemoveInactiveSessions");
            ...
            try
            {
                ...
                try
                {
                    ...
                    string connectionString = Convert.ToString(ConfigurationSettings.AppSettings["SessionConnectionString"]);
            ...
            string sessionId)
        {
            ...
            TraceState trState = trace.StartProc("SessionTable.Remove SessionId=" + sessionId);
            try
            {
                // Record session locally
                Int32 WaitInterval = Convert.ToInt32(ConfigurationSettings.AppSettings["WaitInterval"]);
