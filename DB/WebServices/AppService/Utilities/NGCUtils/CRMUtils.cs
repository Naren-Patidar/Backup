...
namespace Tesco.NGC.Utils
{
    ...
    public class CrmUtils
    {
        ...
        public static string GetEmailAddress(String userName)
        {
            ...
            try
            {
                SearchResultCollection objGrpColl = null;
                ResultPropertyCollection objGrpPropColl = null;
                String strLDAPPath = Convert.ToString(ConfigurationSettings.AppSettings["LDAPPath"]);
                DirectoryEntry objEntry = new DirectoryEntry(strLDAPPath);                
                DirectorySearcher search = new DirectorySearcher(objEntry, "(sAMAccountName= " + userName + ")", new string[] { "mail" }, SearchScope.Subtree);                
                objGrpColl = search.FindAll();

                if (objGrpColl.Count <= 0)
                {
                    emails = null;
                }
                else
                {
                    foreach (SearchResult objUser in objGrpColl)
                    {
                        objGrpPropColl = objUser.Properties;
                        string UserEmail = objGrpPropColl["mail"][0].ToString();
                        emails = UserEmail;
                    }
                }
            }
        ...
        static DirectoryEntry createDirectoryEntry()
        {
            ...
            DirectoryEntry ldapConnection = new DirectoryEntry(LDAPBaseDN);
            ldapConnection.Path = strLDAPPath;
