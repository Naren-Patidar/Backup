...
namespace Tesco.NGC.Utils
{
	...
	public class WebException
	{
		...
		private void HandleException () {
			...
			// Do the tracing stuff.
			TraceState ts = trace.StartProc("WebException.HandleException");
			try {
			...
			) 
		{
			...
			try 
			{
				...
				if (dr != null) 
				{
                    ...
                    try { redirectUrl = dr[columnRedirectUrl].ToString(); }	catch {}
					try { resourceIndex = dr[columnResourceIndex].ToString(); }	catch {}
					try { publish = bool.Parse(dr[columnPublish].ToString()); }	catch {}
