namespace InstoreClubcardReward.Business
{
    ...
    public class Voucher : BaseClass
    {
            ...
            if (CheckValidLength())
            {
                try 
	            {
                    ...
                    if (ConfigurationManager.AppSettings["Country"] == "UK")
                    {
                        ...
                        Type == VoucherTypes.StaffOffer || Type == VoucherTypes.XmasClub)
                        {
                            if (Type == VoucherTypes.XmasClub)
                            {
                                int IsPresent = ValidateBonusVouchers(Ean);
                                if (IsPresent == 1)
                                {
                                    return true;
                                }
        ...
        public int ValidateBonusVouchers(string VoucherNumber)
        {
            ...
            try
            {
                InstoreClubcardReward.Data.ValidateBonusVouchers vb = new InstoreClubcardReward.Data.ValidateBonusVouchers(ConnectionString);
                vb.VoucherNumber = VoucherNumber;

                vb.Execute();

                IsPresent = (int)vb.IsPresent;
            }
            catch (Exception ex)
            {
                throw new BookingException(ErrorTypes.ValidateBonusVouchers, string.Format("Bonus Voucher {0}", VoucherNumber), ex);
            }
