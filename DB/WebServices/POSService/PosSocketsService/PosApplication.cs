...
using Fujitsu.eCrm.Generic.PosService.Security;
...
namespace Fujitsu.eCrm.Seoul.PosSocketsService {
	...
	internal class PosApplication {
		...
		internal static void OnStart(System.Diagnostics.EventLog posSocketsEventLog) {
			...
			// Try to initialise
			while (!PosApplication.InitialiseServer()){
		...
		public static bool InitialiseServer() {
			...
			// the future time determines how far in the future a transaction can be yet still be allowed
            string retryVoucherRedeemTimeString = (string)ConfigurationSettings.AppSettings[posRetryVoucherRedeemTime];
			try {
				// does the string represent an integer
			...
			#region Future Time
			// the future time determines how far in the future a transaction can be yet still be allowed
			futureTimeString = (string)ConfigurationSettings.AppSettings[posFutureTimeSetting];
			try {
				// does the string represent an integer
            ...
            informConversionTxnThreshold = (string)ConfigurationSettings.AppSettings[posInformConversionTxnThresholdSetting];
			try {
				// does the string represent an integer
			...
			string refreshFrequencyString = (string)ConfigurationSettings.AppSettings[posRefreshFrequencySetting];
			try {
		...
		private static bool FinaliseServer() {
