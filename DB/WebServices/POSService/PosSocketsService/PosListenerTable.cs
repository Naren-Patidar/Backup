			...
			lock (this) {
				...
				try {
					...
					string findNgcPortStatement =
                        "SELECT DISTINCT ns.Port ngc_server_port " +
                        "FROM StoreIPAddress ns " +
                        "WHERE ns.Port IS NOT NULL AND ServerType = 'NGC'";
                   
                    

                    string findStoreStatementTemplate =
                        "SELECT DISTINCT " +
                        "ns.IPAddress store_pos_server_ip_address, " +
                        "s.TescoStoreID store_code " +
                        "FROM " +
                        "StoreIPAddress ns, " +
                        "TescoStore s " +
                        "WHERE " +
                        "ns.TescoStoreID = s.TescoStoreID ";
                    ...
                    string connectionString = (string)ConfigurationSettings.AppSettings["AdminConnectionString"];
					...
					try {
						...
						while (ngcPortReader.Read()) {
                            ...
                            //commented because the table is no lnoger used in V3
                    ...
                    // changes are part of code convergence
