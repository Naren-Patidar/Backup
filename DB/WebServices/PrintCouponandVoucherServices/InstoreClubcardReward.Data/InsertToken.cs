...
{
	#region usp_InsertToken Wrapper
    /// <summary>
    /// This class is a wrapper for the usp_InsertToken stored procedure.
    /// </summary>
    public class InsertToken
    {
		...
		public InsertToken(string connectionString)
		{
			this.ConnectionString = connectionString;
		}
		
		public InsertToken(SqlConnection connection)
		{
			this.Connection = connection;
		}
		
		public InsertToken(SqlConnection connection, SqlTransaction transaction)
		{
			this.Connection = connection;
		...
		/// <summary>
		/// The connection string to use when executing the usp_InsertToken stored procedure.
		/// </summary>
        ...
        /// <summary>
        /// The connection to use when executing the usp_InsertToken stored procedure.
        ...
        /// <summary>
        /// The transaction to use when executing the usp_InsertToken stored procedure.
		...
		/// <summary>
		/// Gets the return value from the usp_InsertToken stored procedure.
		/// </summary>
		...
		/// <summary>
		/// Gets the number of rows changed, inserted, or deleted by execution of the usp_InsertToken stored procedure.
		/// </summary>
		...
		/// <summary>
		/// This method calls the usp_InsertToken stored procedure.
		/// </summary>
		public virtual void Execute()
		{
			...
			try
			{
				...
				cmd.CommandText = "[dbo].[usp_InsertToken]";
				...
				if (_tokenIdSet == true)
				{
				prmTokenId.Direction = ParameterDirection.InputOutput;
				}
				else
				{
					prmTokenId.Direction = ParameterDirection.Output;
				}
				...
				if (prmTokenId != null && prmTokenId.Value != null)
				{
					if (prmTokenId.Value is SqlInt32)
					{
						this.TokenId = (SqlInt32)prmTokenId.Value;
					}
					else
					{
						if (prmTokenId.Value != DBNull.Value)
						{
							this.TokenId = new SqlInt32((int)prmTokenId.Value);
						}
						else
						{
							this.TokenId = SqlInt32.Null;
						}
					}
				}
				else
		    ...
		    )
		{
			...
			insertToken.ConnectionString = connectionString;
			insertToken.BookingId = bookingId;
			insertToken.ProductLineId = productLineId;
