...
namespace Tesco.ClubcardProducts.MCA.API.Common.Entities.Boost
{
    public class Token : ComparableEntity<Token>
    {
        ...
        public string ProductTokenValue { get; set; }
        ...
        internal override bool AreInstancesEqual(Token tokenTarget)
        {
            ...
            return
                (tokenTarget.BookingDate.TryParseDate(out dtTargetBooking)) &&
                ...
                (tokenTarget.ValidUntil.TryParseDate(out dtTargetValid)) &&
    ...
    public class TokenList : BaseEntity<TokenList>
    {
        ...
        public override void ConvertFromXml(string xml)
        {
            ...
            _token = (from t in xDoc.Descendants("TokenDetails")
                      select new Token
                      {
                          BookingDate = t.Element(TokenEnum.BookingDate.ToString()).GetValue<string>().TryParseDate(out dtTemp) ? dtTemp.ToString("o") : String.Empty,
                          TokenID = t.Element(TokenEnum.TokenId.ToString()).GetValue<string>(),
                          ...
                          ValidUntil = t.Element(TokenEnum.ValidUntil.ToString()).GetValue<string>().TryParseDate(out dtTemp) ? dtTemp.ToString("o") : String.Empty,
                          ProductCode = t.Element(TokenEnum.ProductCode.ToString()).GetValue<string>(),
                          ProductTokenValue = t.Element(TokenEnum.ProductTokenValue.ToString()).GetValue<string>(),
