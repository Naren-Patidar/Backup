...
namespace Tesco.ClubcardProducts.MCA.API.Logger
{
    ...
    {
        private readonly NLog.ILogger _logger = LogManager.GetLogger("genericLogger");
                ...
                else
                {
                    switch (logData.Level)
                    {
                        ...
                        case LoggingLevel.error:
                            if (logData.LogEntries.Any(l => l.EntryType == LogEntryType.error))
                            {
                                logData.RecordStep("Exiting...");
                                _logger.Info(this.GetFormattedMessage(logData));
                            }
            ...
            try
            {
                if (logData.LogEntries != null && logData.LogEntries.Count > 0)
                {
                    logData.LogEntries.ForEach(l => sbLogData.AppendLine
                                    (
                                        String.Format("{0}|{1}|{2}|{3}|{4}", l.DateTime, l.EntryType.ToString(), logData.Source, l.ErrorCode, l.Message
                                    )));
                }
            }
            catch { }
